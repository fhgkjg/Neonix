// server.js — NEONIX | XENOVA Studios (v4 — کامل و اصلاح‌شده)
...
function publicProfile(u) {
    return {
        ...
        wallet: u.wallet || 0, skins: u.skins || null, skinsEq: u.skinsEq || null
    };
}
...
function onMessage(client, msg) {
    ...
    if (msg.type === 'ping') client.send({ type: 'ping', cb: cb, data: Date.now() });
        ...
        const room = {
            code: code,
            name: d.serverName || ('سرور ' + d.name),
            hostId: client.id, hostName: d.name,
            isPrivate: !!d.isPrivate,
            maxPlayers: Math.min(20, Math.max(2, d.maxPlayers || 2)),
            players: [{ id: 1, socketId: client.id, name: d.name, color: d.color, ready: false, role: 'operator' }],
    ...
    // ✅ اصلاح‌شده: ready ست می‌شه + شروع از ۲ نفر به بالا
        ...
        if (p) {
            p.ready = !!d;
            sendRoomState(room);
            if (room.players.length >= 2 && room.players.every(x => x.ready)) startGame(room);
        }
    }

    // ✅ اصلاح‌شده: >= 2 برای بتل رویال
        ...
        db[key] = {
            ...
            stats: {
                wallet: 0,
                skins: { trail: ['default'], bullet: ['default'], death: ['default'] },
                skinsEq: { trail: 'default', bullet: 'default', death: 'default' },}
        };
        saveUsersDB();
        ...
        if (typeof d.wallet === 'number') found.wallet = d.wallet;
        if (d.skins) found.skins = d.skins;
        if (d.skinsEq) found.skinsEq = d.skinsEq;
        saveUsersDB();
        ...
        for (const name of Object.keys(nets)) {
            for (const net of nets[name]) {
                if (net.family === 'IPv4' && !net.internal) {
                    console.log('📱 آدرس LAN برای گوشی/لپ‌تاپ:  http://' + net.address + ':' + PORT);
                }
            }
        }
    } catch (e) {}
