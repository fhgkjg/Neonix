// ==================== System safety spawn ====================
function isSpawnClear(x, y, r) {
    const cv = (typeof GameState !== 'undefined' && GameState.canvas) ? GameState.canvas : canvas;
    const obs = (typeof GameState !== 'undefined' && GameState.currentObstacles) ? GameState.currentObstacles : currentObstacles;
    if (x - r < 0 || x + r > cv.width || y - r < 0 || y + r > cv.height) return false;
    for (const o of obs) {
        if (circleRectCollision(x, y, r + 4, o)) return false;
    }
    return true;
}

function findSafeSpawn(baseX, baseY, r) {
    // ۱) اگه خود نقطه آزاده، همون
    if (isSpawnClear(baseX, baseY, r)) return { x: baseX, y: baseY };
    // ۲) جستجوی حلقه‌ای اطراف نقطه
    for (let ring = 1; ring <= 12; ring++) {
        const s = ring * 20;
        const candidates = [
            [baseX + s, baseY], [baseX - s, baseY], [baseX, baseY + s], [baseX, baseY - s],
            [baseX + s, baseY + s], [baseX - s, baseY - s], [baseX + s, baseY - s], [baseX - s, baseY + s]
        ];
        for (const c of candidates) {
            if (isSpawnClear(c[0], c[1], r)) return { x: c[0], y: c[1] };
        }
    }
    // ۳) در نهایت تصادفی کل نقشه
    const cv = (typeof GameState !== 'undefined' && GameState.canvas) ? GameState.canvas : canvas;
    for (let t = 0; t < 100; t++) {
        const cx = 40 + Math.random() * (cv.width - 80);
        const cy = 40 + Math.random() * (cv.height - 80);
        if (isSpawnClear(cx, cy, r)) return { x: cx, y: cy };
    }
