let mouseAim = true;
const mousePos = { x: 0, y: 0, down: false, inside: false };
(function () {
    ...
    cv.addEventListener('mouseleave', () => { mousePos.inside = false; });
    cv.addEventListener('mousedown', (e) => { if (e.button === 0) mousePos.down = true; });
...
function separatePlayers() {
    ...
    const free = (pl, x, y) => {
        if (x < pl.radius || x > CONFIG.CANVAS_WIDTH - pl.radius || y < pl.radius || y > CONFIG.CANVAS_HEIGHT - pl.radius) return false;
        for (const o of GameState.currentObstacles) if (circleRectCollision(x, y, pl.radius, o)) return false;
        return true;
    };
    const x1 = p1.x - nx * push, y1 = p1.y - ny * push;
    const x2 = p2.x + nx * push, y2 = p2.y + ny * push;
    if (free(p1, x1, y1)) { p1.x = x1; p1.y = y1; }
    if (free(p2, x2, y2)) { p2.x = x2; p2.y = y2; }
}
function handleInput() {
    ...
    if (onlineMode) {
        ...
        if (mouseAim && mousePos.inside) me.angle = Math.atan2(mousePos.y - me.y, mousePos.x - me.x);
        if (GameState.keys['Space'] || GameState.keys['Enter'] || mousePos.down) me.shoot();
        return;
    }
    ...
    if (mouseAim && mousePos.inside) GameState.p1.angle = Math.atan2(mousePos.y - GameState.p1.y, mousePos.x - GameState.p1.x);
    if (GameState.keys['Space'] || mousePos.down) GameState.p1.shoot();
...
function update() {
    if (!GameState.gameStarted || GameState.gameOver || GameState.gamePause) return;
    handleInput();
    if (!onlineMode) separatePlayers();
    ...
    for (let i = GameArrays.bullets.length - 1; i >= 0; i--) {
        ...
        for (let j = GameArrays.stars.length - 1; j >= 0; j--) {
            ...
            if (Math.hypot(b.x - s.x, b.y - s.y) < s.radius + 8) {
                ...
                if (nearest) {
                    ...
                    addFloatingText(nearest.x, nearest.y - 20, `+15 ${t('fx.heal')}`, "#00ff00");
                    playSound('heal');
                }
        ...
        if (circleRectCollision(b.x, b.y, 4, { x: target.x - target.radius, y: target.y - target.radius, w: target.radius * 2, h: target.radius * 2 })) {
            ...
            if (isDead) {
                ...
                if (typeof spawnSkinDeathEffect === 'function') spawnSkinDeathEffect(target.x, target.y, target.color);
                else for (let k = 0; k < 30; k++) spawnParticle(target.x, target.y, target.color, 'explosion');
    ...
    for (let i = GameArrays.shields.length - 1; i >= 0; i--) {
        ...
        if (checkCircleCollision(GameState.p1, sh)) {
            ...
            addFloatingText(GameState.p2.x, GameState.p2.y - 20, t('fx.shield'), "#8a2be2");
            playSound('shield');
            GameArrays.shields.splice(i, 1);
        }
    }
        ...
        const applyPowerup = (player) => {
            switch(pu.type) {
                case 'speed':
                    ...
                    addFloatingText(player.x, player.y - 20, t('fx.speed'), "#ffff00");
                    break;
                case 'triple':
                    player.tripleShot = Date.now() + 10000;
                    addFloatingText(player.x, player.y - 20, t('fx.triple'), "#ff00ff");
                    break;
                case 'rapid':
                    player.rapidFire = Date.now() + 7000;
                    addFloatingText(player.x, player.y - 20, t('fx.rapid'), "#ff6600");
                    break;
                case 'invis':
                    player.invisibility = Date.now() + 5000;
                    addFloatingText(player.x, player.y - 20, t('fx.invisible'), "#ffffff");
                    break;
            }
            playSound('pickup');
            player.addXP(20);
    ...
    for (let i = GameArrays.floatTexts.length - 1; i >= 0; i--) {
        const ft = GameArrays.floatTexts[i];
        ft.y -= 0.8;
        ft.life -= 0.02;
        if (ft.life <= 0) GameArrays.floatTexts.splice(i, 1);
    }
...
function draw() {
    ...
    for (let b of GameArrays.bullets) {
        if (typeof drawSkinBullet === 'function') drawSkinBullet(ctx, b);
        else {
    ...
    for (const ft of GameArrays.floatTexts) {
...
function gameLoop() {
    ...
    try {
        ...
        if (!gameLoop._errLogged) { gameLoop._errLogged = true; console.error('❌ draw error:', e); }
    }
...
function startGame(mode) {
    ...
    if (mode === 'bot') {
        const oldSel = document.getElementById('bot-diff-select');
        if (oldSel) GameState.botDifficulty = oldSel.value;
        // 👇 ترجمه نام ربات
        const diffNames = {
            easy: t('mode.easy'),
...
function endGame(winnerId) {
    ...
    `;
    const walletGain = (typeof payWalletOnce === 'function') ? payWalletOnce() : 0;
    if (walletGain > 0) statsHTML += '<div class="stat-box"><div class="stat-value">+' + walletGain + '</div><div class="stat-label">💰 کیف پول اسکین</div></div>';
...
function resetGame() {
    ...
    GameState.gameOver = false;
    GameState.gameStarted = true;
    const s1 = findSafeSpawn(60, CONFIG.CANVAS_HEIGHT / 2, CONFIG.PLAYER_RADIUS);
    const s2 = findSafeSpawn(CONFIG.CANVAS_WIDTH - 60, CONFIG.CANVAS_HEIGHT / 2, CONFIG.PLAYER_RADIUS);
    GameState.p1 = new Player(1, s1.x, s1.y, GameState.p1Color);
    GameState.p2 = new Player(2, s2.x, s2.y, GameState.p2Color);
