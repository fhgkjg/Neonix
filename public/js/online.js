// ==================== online.js — NEONIX (v4 — کامل و اصلاح‌شده) ====================
...
function connectToServer() {
    ...
    socket.on('connect', () => { addChatMessage(t('chat.system'), t('chat.connected'), '#00ff88'); startPingCheck(); });
    ...
    socket.on('room:player_left', (d) => {
        addChatMessage(t('chat.system'), d.name + ' ' + t('chat.left'), '#ff416c');
        if (typeof brActive !== 'undefined' && brActive) return; // بتل رویال خودش مدیریت می‌کنه
        if (onlineMode) { onlineMode = false; showOnlineMessage('error', d.name + ' ' + t('chat.left')); returnToOnlineMenu(); }
    });
    socket.on('room:kicked', () => { onlineMode = false; showOnlineMessage('error', t('wr.kicked')); returnToOnlineMenu(); });
    // ✅ فقط یک هندلر — با شاخه بتل رویال
    socket.on('game:start', (d) => {
        if (d.players && d.players.length > 2 && typeof startBattleRoyaleOnline === 'function') {
            startBattleRoyaleOnline(d.players);
        } else {
            document.getElementById('waiting-room').style.display = 'none';
            startOnlineGame(d.players);
        }
    });
...
// ✅ اصلاح‌شده: maxPlayers داخل emit
function createServer() {
    ...
    socket.emit('room:create', {
        name: name,
        color: GameState.p1Color,
        serverName: serverName,
        isPrivate: isPrivate,
        customCode: customCode,
        maxPlayers: window.pendingMaxPlayers || 2
    }, (res) => {
        if (res.success) {
...
function joinRoom(code) {
    const name = getMyName();
    socket.emit('room:join', { code: code, name: name, color: GameState.p1Color }, (res) => {
        if (res.success) {
            myPlayerId = res.playerId;
            isHost = false;
            document.getElementById('online-menu').style.display = 'none';
...
function joinRoomByCode() {
    ...
    if (code.length < 4) { showOnlineMessage('error', t('error.invalid_code')); return; }
    joinRoom(code);
}
...
function renderWaitingRoom(room) {
    ...
    if (me && me.role === 'operator') { roleEl.innerText = t('wr.operator'); roleEl.className = 'role-badge operator'; }
    else { roleEl.innerText = t('wr.guest'); roleEl.className = 'role-badge guest'; }
...
function kickPlayer(playerId) {
    if (!isHost) return;
    if (confirm(t('misc.confirm'))) socket.emit('room:kick', { playerId: playerId });
}
...
function inviteFriend() {
    const code = document.getElementById('room-code-display').innerText;
    const text = '⚡ NEONIX! ' + t('wr.code_hint') + ' ' + code;
