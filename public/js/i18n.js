...
function applyTranslations() {
    ...
    document.querySelectorAll('select[data-i18n-options]').forEach(select => {
        ...
        Array.from(select.options).forEach(opt => {
            if (!opt.value) return;
            const withDot = keyBase + '.' + opt.value;
            const withUnder = keyBase + '_' + opt.value;
            let tr = t(withDot);
            if (tr === withDot) tr = t(withUnder);   // اگه پیدا نشد، فرمت خط‌زیره
            if (tr !== withDot && tr !== withUnder) opt.textContent = tr;
        });
    });
    
    console.log(`languages chage it: ${currentLanguage.toUpperCase()}`);
}
